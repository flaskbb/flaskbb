...
class TestPasswordReset(object):
    def test_raises_token_error_if_not_a_password_reset(self, token_serializer):
        ...
        with pytest.raises(TokenError) as excinfo:
            service.reset_password(raw_token, "some@e.mail", "a great password!")
    ...
    def test_raises_StopValidation_if_verifiers_fail(self, token_serializer):
        ...
        service = password.ResetPasswordService(token_serializer, User, [verifier])
    ...
    def test_initiate_raises_if_user_doesnt_exist(self, token_serializer, Fred):
        service = password.ResetPasswordService(token_serializer, User, [])
    ...
    def test_calls_send_reset_token_successfully_if_user_exists(
        ...
        mocker.patch("flaskbb.auth.services.password.send_reset_token.delay", mock)
